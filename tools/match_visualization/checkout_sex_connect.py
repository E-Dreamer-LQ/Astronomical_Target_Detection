...
def sex_list_match(sex_img,list_img,section_target_num):
    ...
    for i in range(det_target):
        x_axis = np.around(sex_img[i][1])
        y_axis = np.around(sex_img[i][2])
    ...
    if number_target != 0:
        recall = match_num/number_target
    else:
        recall = 1
        precision = 1
...
def det_list_match(det_img,list_img,section_target_num):
    ...
    match_value = 1.5
    for i in range(det_target):
        x_axis = np.around((det_img[i][0]+det_img[i][2])/2)
        y_axis = np.around((det_img[i][1]+det_img[i][3])/2)
        if match_num <= number_target:
            for j in range(number_target):
                if abs(x_axis - list_img[j][1]) <= match_value and abs(y_axis - list_img[j][2]) <= match_value:
                    match_num += 1
...
print("completed!!!")
    ...
    # plt.bar(x, precision_all_det, alpha=0.9, width=0.35, facecolor='lightskyblue', edgecolor='white', label='precision_all_det', lw=1)
    # plt.bar(x + 0.35, precision_all_sex, alpha=0.9, width=0.35, facecolor='yellowgreen', edgecolor='white', label='precision_all_sex', lw=1)
    # plt.legend(loc="upper right")  # label的位置在左上，没有这句会找不到label去哪了
    # plt.savefig('/home/lab30202/lq/ai_future/match_visualization/precision_compare.jpg')
    # plt.close()
    #
    # plt.figure(figsize=(9, 6))
    # plt.bar(x, recall_all_det, alpha=0.9, width=0.35, facecolor='lightskyblue', edgecolor='white', label='recall_all_det', lw=1)
    # plt.bar(x + 0.35, recall_all_sex, alpha=0.9, width=0.35, facecolor='yellowgreen', edgecolor='white', label='recall_all_sex', lw=1)
    # plt.legend(loc="upper right")  # label的位置在左上，没有这句会找不到label去哪了
    # plt.savefig('/home/lab30202/lq/ai_future/match_visualization/recall_compare.jpg')
    # plt.close()
    #
    #
